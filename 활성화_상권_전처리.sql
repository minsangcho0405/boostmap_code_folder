...
--   - 상권_코드, 기준_년분기_코드, 총_유동인구_수 컬럼을 가진 테이블 가정
-- ============================================================
...
with_yoy AS (
    SELECT
        ...
        CASE
            ...
            WHEN LAG(매출_금액, 4) OVER (PARTITION BY 상권_코드 ORDER BY 분기순번) < 5000000 THEN NULL  -- 500만원 미만 제외
...
with_cagr AS (
    SELECT
        ...
        END AS 누적4분기_성장률
    FROM with_prev4 w
    WHERE 분기순번 >= 8   -- CAGR 계산에 필요한 8분기 이력 제약: 분기순번 1~7 항상 제외
),
...
-- ------------------------------------------------------------
-- STEP 8. 최종 6개 피처 (t검정 유의 피처만 채택)
--   ① 매출_저점대비_반등폭 = 현재YoY - 최근3분기 중 최저YoY
--      ※ 마스터 데이터 실측값과 대조 시 42~77%만 일치, 정확한 원본 공식 미확인 (검증 필요)
--   ② 매출_모멘텀 = 최근2분기 평균YoY - 그 이전2분기 평균YoY (마스터 데이터 100% 일치 검증됨)
--   ③ 분기별_총_유동인구_수 (원본 그대로, STEP 3에서 이미 병합됨)
--   ④ 구매전환율_100cap(%) = min(매출건수/유동인구×100, 100)
--   ⑤ 저녁심야_매출_비중(%) = 저녁심야매출 / 전체매출 × 100
--   ⑥ `2030대_소비_비중` = 2030매출 / 전체매출 (0~1 fraction, ×100 안 함 — 마스터 데이터 관례)
-- ------------------------------------------------------------
with_features AS (
    SELECT
        w.*,

        -- ① 매출_저점대비_반등폭 (공식 미검증, 아래 STEP 8 상단 주석 참고)
        (w.매출_YoY_윈저화 - LEAST(
        ...
        -- ② 매출_모멘텀 = 최근2분기 평균YoY - 이전2분기 평균YoY
        (
        ...
        -- ⑥ `2030대_소비_비중` (마스터 데이터 관례상 0~1 fraction으로 저장, ×100 하지 않음)
...
--   train/validation/test/apply 분할은 분기순번 기준으로 애플리케이션/BI 단에서
--   CASE WHEN 분기순번 BETWEEN 8 AND 11 THEN 'train' ... 식으로 별도 부여 권장
-- ------------------------------------------------------------
final AS (
    SELECT
        w.상권_코드,
...
SELECT *
FROM final
WHERE 활성화_현재상태 = 0   -- 예측 대상: 현재 비활성화 상권만
ORDER BY 상권_코드, 분기순번;
