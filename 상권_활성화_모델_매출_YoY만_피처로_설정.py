...
def load_historical_data():
    target_file = os.path.join(BASE_DIR, "데이터_최종_활성화라벨_CAGR규모필터_4.xlsx")
...
# 비교모델 채택 피처(1개) — 6개 피처 모델과의 성능 비교용 베이스라인.
# t검정 결과 Cohen's d가 가장 큰 단일 피처(매출_YoY_윈저화)만 단독으로 사용함.
FINAL_FEATURES = [
    "매출_YoY_윈저화(%)",
]
...
if __name__ == "__main__":
    ...
    ttest_out_path = os.path.join(BASE_DIR, "t검정_결과.xlsx")
    ttest_result.to_excel(ttest_out_path, index=False)
    ...
    print(f"\n=== 매출 YoY 단일 피처 모델 test셋 성능 ===")
    print(f"AUC: {auc_test:.4f}")
    print(f"F1 : {f1_test:.4f}")

    # 3) 예측 및 저장
    print("\n3) apply_최종예측대상 예측 순위 산출 중...")
    ranking_df = predict_apply_target(full, model, scaler)

    ranking_out_path = os.path.join(BASE_DIR, "예측_순위표_매출YoY단일.xlsx")
    ranking_df.to_excel(ranking_out_path, index=False)

    print("\n=== 매출 YoY 기준 활성화 가능성 예측 Top 5 상권 ===")
    print(ranking_df[["순위", "상권_코드_명", "예측확률", "매출_YoY_윈저화(%)"]].head().to_string(index=False))

    print(f"\n파일 저장 완료:")
    print(f" - {ttest_out_path}")
    print(f" - {ranking_out_path}")
